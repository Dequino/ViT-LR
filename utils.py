...
def confusion_matrix(model, criterion, batch_size, test_x, test_y, use_cuda=True,
                 mask=None, preproc=None):
    model.eval()
    ...
    for i in range(test_it):
        ...
        loss = criterion(logits, y)
        _, pred_label = torch.max(logits.data, 1)
        pred_labels_all = np.append(pred_labels_all, pred_label.cpu(), axis=0)
        correct_cnt += (pred_label == y.data).sum()
        ave_loss += loss.item()
    ...
    y_actu = pd.Series(actual_labels_all, name='Actual')
    y_pred = pd.Series(pred_labels_all, name='Predicted')
...
def return_mistake(model, batch_size, test_x, test_y, use_cuda=True,
                 mask=None, preproc=None):
    model.eval()
    model = maybe_cuda(model, use_cuda=use_cuda)
    ...
    for i in range(test_it):
        ...
        for j, pred in enumerate(pred_label):
                print("pred:"+str(pred)+"y:"+str(y.data[j]))
                if pred != y.data[j]:
                        count += 1
                        if count == 10:
